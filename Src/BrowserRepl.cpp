...
void BrowserRepl::RegisterACAPIJavaScriptObject(DG::Browser& browser)
{
    ...
    // ------------------------------------------------------------
    // ResetCassetteSettings - сбросить к умолчаниям
    // ------------------------------------------------------------
    
    jsACAPI->AddItem(new JS::Function("ResetCassetteSettings", [](GS::Ref<JS::Base>) -> GS::Ref<JS::Base> {
        CassetteSettings::Settings defaults = CassetteSettings::GetDefaultSettings();
        bool success = CassetteSettings::SaveSettings(defaults);
